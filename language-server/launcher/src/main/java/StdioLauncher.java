...
/**
 * Launcher for silver language server.
 */
public class StdioLauncher {
    ...
    private static void startServer(InputStream in, OutputStream out) throws ExecutionException, InterruptedException {
        // Initialize the SilverLanguageServer
        SilverLanguageServer silverLanguageServer = new SilverLanguageServer();
        // Create JSON RPC launcher for SilverLanguageServer instance.
        Launcher<LanguageClient> launcher = LSPLauncher.createServerLauncher(silverLanguageServer, in, out);

        // Get the client that request to launch the LS.
        LanguageClient client = launcher.getRemoteProxy();

        // Set the client to language server
        silverLanguageServer.connect(client);
