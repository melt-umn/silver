...
This tutorial guides you through building Monto 'servers' with Silver. If you are not familiar with the Monto DDE, you can read about and download the latest version of Monto here: https://bitbucket.org/inkytonik/monto/overview. You should be familiarize yourself with the general architecture of a Monto environment as well as the format of JSON messages sent between Monto components.

Included in the tutorial is a language for evaulating simple mathematical expressions. The language is defined in the abstractsyntax, concretesyntax, and driver directories. Generally, the files in these directories should not be modified.
...
We have also defined a function called evaluate in the Main.sv file. This function is passed to the server and is what the server uses to create products out of messages. The server expectes this function to accept a single MontoMessage as a parameter and return a MontoProduct. To extract the source, language, contents, and selections from a MotoMessage, we can use the getSource, getLanguage, getContents, and getSelections functions respectivly. These functions are defined in lib:monto. The evaluate function uses the components extracted from the MontoMessage, along with the driverEval function defined in monto:driver, to create and return a MontoProduct.
